...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/user/avatar", authenticateJWT, async (req, res) => {
    ...
    try {
      ...
      // Validate avatar is a base64 string
      if (!avatar || typeof avatar !== "string") {
        return res.status(400).json({ message: "Avatar must be a base64 string" });
      }

      // Calculate approximate decoded size of base64 string in bytes
      const base64SizeBytes = (avatar.length * 3) / 4 - (avatar.endsWith("==") ? 2 : avatar.endsWith("=") ? 1 : 0);
      console.log(`Avatar base64 size: ${base64SizeBytes} bytes (~${(base64SizeBytes / 1024).toFixed(2)} kB)`);

      // Enforce 5MB limit
      const MAX_SIZE_BYTES = 5 * 1024 * 1024; // 5MB in bytes
      if (base64SizeBytes > MAX_SIZE_BYTES) {
        return res.status(400).json({ message: "Avatar exceeds 5MB limit" });
      }
